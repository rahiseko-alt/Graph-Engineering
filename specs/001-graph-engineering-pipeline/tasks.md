...
- [X] T009 [P] [US1] 開いた pull request の一覧と、各 pull request が担当する作業単位の対応を `docs/measurements/<対象名>-graph.md` に記録する
...
- [X] T015 [US1] 成功済みの作業単位が再実行されないことを確認して記録する（SC-005）。**実際にやったこと**: 単位Aを統合した後、単位Bの修正と統合を行う間、単位Aを一度も再実行しなかったことをコミット数で確認した。**プロセスを強制終了して再開する形の中断は試していない**
