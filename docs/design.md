...
### 2026-08-28 門②の結果と、全7件の扱い

別エージェントによる7問のずれ検知で、**7問すべてが「はい」**になった。検知は握りつぶさない（検知の原則5）。全件の扱いを残す。

| # | 指摘 | 扱い | 内容 |
|---|---|---|---|
| Q3 | 中断からの再開を試していないのに成立と記録した | **採用・修正** | 実際に SIGKILL で強制終了して再開する試験を行った。成功済み2単位は SKIP、中断した単位から再開。記録を実測へ差し替えた |
| Q3 | 比較相手の単一AI版がリポジトリに残っておらず再実行できない | **採用・修正** | `sandbox/business-days-single/` として凍結保存した。同じ受入検査を当てて 5/5 通過を再現済み |
| Q3 | 「開いた申請ごとに記録した」が、申請は1件も存在しない | **採用・修正** | 該当タスクの記述を、実際に行ったこと（ブランチを単位の境界にした）へ書き直した |
| Q4 | 使う量を減らす狙いが、実測では逆方向の結果になった | **採用・記録** | CA-003 を **偽（この規模では）** に変更した。実装の修正ではなく、上位の判断へ戻す材料として扱う |
| Q1 | 依頼にない「営業日を数えるコマンド」を一式作り、差分の大半を占める | **却下（理由あり）** | SC-003 は同じ依頼を2通りで実行して比較することを求めており、比較の対象が要る。工程で工程を作ると詰まりの原因を切り分けられないため、工程と無関係の対象を選ぶ必要があった。ただし**差分の大半を占めているのは事実**であり、Q5 として別に扱う |
| Q2 / Q5 | 出来上がったものの大半が文書で、動くものが増えていない | **採用・報告** | 事実。避けるべき未来1位に該当する。次の作業を増やさず、依頼者へ報告して判断を仰ぐ |
| Q2 / Q7 | 割り当ての実走・検査の自動実行・他AIでの通し確認が未実施 | **限界として記録** | いずれもこのセッションの制約（該当機能が提供されていない、指定ブランチ以外へプッシュできない）による。環境が変われば実施できる |
| Q6 | 続けるか・次にどの大きさで測るか・使う量をどう数えるかが判断待ち | **採用・報告** | 依頼者の判断が要る。手が実際に止まる箇所であり、報告する |
